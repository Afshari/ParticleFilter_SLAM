...
void host_bresenham_2();
void host_update_map_2();
void host_map_2();
...
MapData map_data;
MapData map_data_bg;
MapData map_data_post; 
GeneralInfo general_info; 
Measurements measurements;
ParticlesData particles_data; 
PositionTransition position_transition;
...
int test_map_main() {
    ...
    read_update_map(720, map_data, map_data_bg,
        map_data_post, general_info, measurements, particles_data, position_transition);
...
//
//    int xmin = map_data.xmin;
//    int xmax = map_data.xmax;
//    int ymin = map_data.ymin;
//    int ymax = map_data.ymax;
//
//    int xmin_pre = xmin;
//    int ymax_pre = ymax;
//
...
//    LIDAR_COORDS_LEN = measurements.LIDAR_COORDS_LEN;
//
//    /********************* IMAGE TRANSFORM VARIABLES ********************/
...
//
//    gpuErrchk(cudaMemcpy(d_lidar_coords, measurements.lidar_coords.data(), sz_lidar_coords, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_transition_body_lidar, h_transition_body_lidar, sz_transition_body_lidar, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_transition_single_world_body, position_transition.transition_world_body.data(), sz_transition_single_frame, cudaMemcpyHostToDevice));
//
//
//    sz_lidar_coords = 2 * LIDAR_COORDS_LEN * sizeof(float);
...
//
//    gpuErrchk(cudaMemcpy(d_lidar_coords, measurements.lidar_coords.data(), sz_lidar_coords, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_grid_map, map_data.grid_map.data(), sz_grid_map, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_log_odds, map_data.log_odds.data(), sz_log_odds, cudaMemcpyHostToDevice));
...
//
//    assert(EXTEND == map_data.should_extend);
//
//    if (EXTEND == true) {
//
...
//
//        kernel_position_to_image << <1, 1 >> > (d_coord, SEP, xmin_pre, ymax_pre, general_info.res, xmin, ymax);
//        cudaDeviceSynchronize();
//
...
//            GRID_WIDTH, GRID_HEIGHT, map_data_post.GRID_WIDTH, map_data_post.GRID_HEIGHT);
//        assert(GRID_WIDTH == map_data_post.GRID_WIDTH);
//        assert(GRID_HEIGHT == map_data_post.GRID_HEIGHT);
//
...
//            if (res_grid_map[i] != map_data_bg.grid_map[i]) {
//                error_map += 1;
//                //printf("Grid Map: %d <> %d\n", res_grid_map[i], bg_grid_map[i]);
//            }
//            if ( abs(res_log_odds[i] - map_data_bg.log_odds[i]) > 1e-4) {
//                error_log += 1;
//                printf("Log Odds: (%d) %f <> %f\n", i, res_log_odds[i], map_data_bg.log_odds[i]);
//            }
//        }
//        printf("Map Erros: %d\n", error_map);
//        printf("Log Erros: %d\n", error_log);
...
//        d_transition_single_world_lidar, d_lidar_coords, general_info.res, xmin, ymax, LIDAR_COORDS_LEN);
//    cudaDeviceSynchronize();
//
//    kernel_position_to_image << < 1, 1 >> > (d_position_image_body, SEP, d_transition_single_world_lidar, general_info.res, xmin, ymax);
//    cudaDeviceSynchronize();
...
//
//    ASSERT_transition_world_lidar(res_transition_world_lidar, position_transition.transition_world_lidar.data() , 9, false);
//    ASSERT_particles_world_frame(res_particles_world_x, res_particles_world_y, 
//        particles_data.particles_world_x.data(), particles_data.particles_world_y.data(), LIDAR_COORDS_LEN, false);
//    ASSERT_processed_measurements(res_processed_single_measure_x, res_processed_single_measure_y,
//        particles_data.particles_occupied_x.data(), particles_data.particles_occupied_y.data(), LIDAR_COORDS_LEN);
//    ASSERT_position_image_body(res_position_image_body, position_transition.position_image_body.data(), true, true);
//
...
void host_update_map_init_2() {
    ...
    device_vector<float> dvec_lidar_coords(2 * LIDAR_COORDS_LEN);
    ...
    device_vector<int> dvec_should_extend(4);

    host_vector<int> hvec_grid_map(GRID_WIDTH * GRID_HEIGHT);
    host_vector<float> hvec_log_odds(GRID_WIDTH * GRID_HEIGHT);

    dvec_lidar_coords.assign(measurements.lidar_coords.begin(), measurements.lidar_coords.end());
    dvec_transition_single_world_body.assign(position_transition.transition_world_body.begin(), position_transition.transition_world_body.end());
    dvec_transition_body_lidar.assign(hvec_transition_body_lidar.begin(), hvec_transition_body_lidar.end());

    dvec_grid_map.assign(map_data.grid_map.begin(), map_data.grid_map.end());
    dvec_log_odds.assign(map_data.log_odds.begin(), map_data.log_odds.end());
    host_vector<int> hvec_should_extend(4, 0);
    ...
    hvec_should_extend.assign(dvec_should_extend.begin(), dvec_should_extend.end());

    bool EXTEND = false;
    if (hvec_should_extend[0] != 0) {
        EXTEND = true;
        xmin = xmin * 2;
    }
    else if (hvec_should_extend[2] != 0) {
        EXTEND = true;
        xmax = xmax * 2;
    }
    else if (hvec_should_extend[1] != 0) {
        EXTEND = true;
        ymin = ymin * 2;
    }
    else if (hvec_should_extend[3] != 0) {
        EXTEND = true;
        ymax = ymax * 2;
    }
    auto stop_check_extend = std::chrono::high_resolution_clock::now();
    ...
    printf("xmin=%d, xmax=%d, ymin=%d, ymax=%d\n", xmin, xmax, ymin, ymax);
    for (int i = 0; i < 4; i++)
        std::cout << "Should Extend: " << hvec_should_extend[i] << std::endl;
    ...
    if (EXTEND == true) {
        ...
        hvec_grid_map.resize(GRID_WIDTH * GRID_HEIGHT, 0);
        hvec_log_odds.resize(GRID_WIDTH * GRID_HEIGHT, 0);
        ...
        hvec_grid_map.assign(dvec_grid_map.begin(), dvec_grid_map.end());
        hvec_log_odds.assign(dvec_log_odds.begin(), dvec_log_odds.end());

        int error_map = 0;
        int error_log = 0;
        for (int i = 0; i < (GRID_WIDTH * GRID_HEIGHT); i++) {
            if (hvec_grid_map[i] != map_data_bg.grid_map[i]) {
                error_map += 1;
                //printf("Grid Map: %d <> %d\n", hvec_grid_map[i], bg_grid_map[i]);
            }
            if (abs(hvec_log_odds[i] - map_data_bg.log_odds[i]) > 1e-4) {
                error_log += 1;
                printf("Log Odds: (%d) %f <> %f\n", i, hvec_log_odds[i], map_data_bg.log_odds[i]);
            }
        }
        printf("Map Erros: %d\n", error_map);
        printf("Log Erros: %d\n", error_log);
    ...
    ASSERT_transition_world_lidar(hvec_transition_world_lidar.data(), position_transition.transition_world_lidar.data(), 9, false);
    ...
    ASSERT_processed_measurements(hvec_processed_single_measure_x.data(), hvec_processed_single_measure_y.data(),
        particles_data.particles_occupied_x.data(), particles_data.particles_occupied_y.data(), LIDAR_COORDS_LEN);
    ASSERT_position_image_body(hvec_position_image_body.data(), position_transition.position_image_body.data(), true, true);
...
//
//    /************************ BRESENHAM VARIABLES ***********************/
//    PARTICLES_OCCUPIED_LEN = particles_data.PARTICLES_OCCUPIED_LEN;
//    PARTICLES_FREE_LEN = particles_data.PARTICLES_FREE_LEN;
...
//
//    gpuErrchk(cudaMemcpy(d_particles_occupied_x, particles_data.particles_occupied_x.data(), sz_particles_occupied_pos, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_particles_occupied_y, particles_data.particles_occupied_y.data(), sz_particles_occupied_pos, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_particles_free_idx, particles_data.particles_free_idx.data(), sz_particles_occupied_pos, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_position_image_body, position_transition.position_image_body.data(), sz_position_image_body, cudaMemcpyHostToDevice));
//
//    /************************* BRESENHAM KERNEL *************************/
...
//
//    ASSERT_particles_free_index(res_particles_free_counter, particles_data.particles_free_idx.data(), PARTICLES_OCCUPIED_LEN, false);
//    ASSERT_particles_free_new_len(PARTICLES_FREE_LEN, particles_data.PARTICLES_FREE_LEN);
//    ASSERT_particles_free(res_particles_free_x, res_particles_free_y, 
//        particles_data.particles_free_x.data(), particles_data.particles_free_y.data(), PARTICLES_FREE_LEN, true, true);
//
...
void host_bresenham_2() {
    ...
    host_vector<int> hvec_particles_free_counter(PARTICLE_UNIQUE_COUNTER, 0);

    device_vector<int> dvec_particles_occupied_x(PARTICLES_OCCUPIED_LEN);
    device_vector<int> dvec_particles_occupied_y(PARTICLES_OCCUPIED_LEN);
    device_vector<int> dvec_particles_free_x_max(PARTICLES_OCCUPIED_LEN * MAX_DIST_IN_MAP, 0);
    device_vector<int> dvec_particles_free_y_max(PARTICLES_OCCUPIED_LEN * MAX_DIST_IN_MAP, 0);
    device_vector<int> dvec_particles_free_counter(PARTICLE_UNIQUE_COUNTER, 0);
    device_vector<int> dvec_particles_free_idx(PARTICLES_OCCUPIED_LEN);
    device_vector<int> dvec_position_image_body(2);

    dvec_particles_occupied_x.assign(particles_data.particles_occupied_x.begin(), particles_data.particles_occupied_x.end());
    dvec_particles_occupied_y.assign(particles_data.particles_occupied_y.begin(), particles_data.particles_occupied_y.end());
    dvec_particles_free_idx.assign(particles_data.particles_free_idx.begin(), particles_data.particles_free_idx.end());
    dvec_position_image_body.assign(position_transition.position_image_body.begin(), position_transition.position_image_body.end());
    ...
    threadsPerBlock = 256;
    blocksPerGrid = (PARTICLES_OCCUPIED_LEN + threadsPerBlock - 1) / threadsPerBlock;
    kernel_bresenham << <blocksPerGrid, threadsPerBlock >> > (THRUST_RAW_CAST(dvec_particles_free_x_max), THRUST_RAW_CAST(dvec_particles_free_y_max), THRUST_RAW_CAST(dvec_particles_free_counter), SEP,
        THRUST_RAW_CAST(dvec_particles_occupied_x), THRUST_RAW_CAST(dvec_particles_occupied_y), THRUST_RAW_CAST(dvec_position_image_body),
        PARTICLES_OCCUPIED_LEN, MAX_DIST_IN_MAP);
    cudaDeviceSynchronize();

    thrust::exclusive_scan(thrust::device, dvec_particles_free_counter.begin(), dvec_particles_free_counter.end(), dvec_particles_free_counter.begin(), 0);
    auto stop_bresenham = std::chrono::high_resolution_clock::now();

    auto start_bresenham_rearrange = std::chrono::high_resolution_clock::now();
    hvec_particles_free_counter.assign(dvec_particles_free_counter.begin(), dvec_particles_free_counter.end());

    PARTICLES_FREE_LEN = hvec_particles_free_counter[PARTICLE_UNIQUE_COUNTER - 1];
    device_vector<int> dvec_particles_free_x(PARTICLES_FREE_LEN);
    device_vector<int> dvec_particles_free_y(PARTICLES_FREE_LEN);

    kernel_bresenham_rearrange << <blocksPerGrid, threadsPerBlock >> > (THRUST_RAW_CAST(dvec_particles_free_x), THRUST_RAW_CAST(dvec_particles_free_y), SEP,
        THRUST_RAW_CAST(dvec_particles_free_x_max), THRUST_RAW_CAST(dvec_particles_free_y_max),
        THRUST_RAW_CAST(dvec_particles_free_counter), MAX_DIST_IN_MAP, PARTICLES_OCCUPIED_LEN);
    cudaDeviceSynchronize();
    auto stop_bresenham_rearrange = std::chrono::high_resolution_clock::now();

    host_vector<int> hvec_particles_free_x(PARTICLES_FREE_LEN);
    host_vector<int> hvec_particles_free_y(PARTICLES_FREE_LEN);

    hvec_particles_free_x.assign(dvec_particles_free_x.begin(), dvec_particles_free_x.end());
    hvec_particles_free_y.assign(dvec_particles_free_y.begin(), dvec_particles_free_y.end());

    ASSERT_particles_free_index(hvec_particles_free_counter.data(), particles_data.particles_free_idx.data(), PARTICLES_OCCUPIED_LEN, false);
    ASSERT_particles_free_new_len(PARTICLES_FREE_LEN, particles_data.PARTICLES_FREE_LEN);
    ASSERT_particles_free(hvec_particles_free_x.data(), hvec_particles_free_y.data(),
        particles_data.particles_free_x.data(), particles_data.particles_free_y.data(), PARTICLES_FREE_LEN, true, true);
...
//
//    LIDAR_COORDS_LEN = measurements.LIDAR_COORDS_LEN;
//    GRID_WIDTH = map_data_post.GRID_WIDTH;
//    GRID_HEIGHT = map_data_post.GRID_HEIGHT;
//
//    PARTICLES_OCCUPIED_LEN = particles_data.PARTICLES_OCCUPIED_LEN;
//    PARTICLES_FREE_LEN = particles_data.PARTICLES_FREE_LEN;
//    PARTICLE_UNIQUE_COUNTER = PARTICLES_OCCUPIED_LEN + 1;
...
//    gpuErrchk(cudaMalloc((void**)&d_particles_free_y, sz_particles_free_pos));
//
//    gpuErrchk(cudaMemcpy(d_grid_map, map_data_bg.grid_map.data(), sz_grid_map, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_particles_occupied_x, particles_data.particles_occupied_x.data(), sz_particles_occupied_pos, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_particles_occupied_y, particles_data.particles_occupied_y.data(), sz_particles_occupied_pos, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_particles_free_x, particles_data.particles_free_x.data(), sz_particles_free_pos, cudaMemcpyHostToDevice));
//    gpuErrchk(cudaMemcpy(d_particles_free_y, particles_data.particles_free_y.data(), sz_particles_free_pos, cudaMemcpyHostToDevice));
//
//
//    /************************* LOG-ODDS VARIABLES ***********************/
...
//    
//    printf("--> Occupied Unique: %d, %d\n", PARTICLES_OCCUPIED_UNIQUE_LEN, particles_data.PARTICLES_OCCUPIED_UNIQUE_LEN);
//    assert(PARTICLES_OCCUPIED_UNIQUE_LEN == particles_data.PARTICLES_OCCUPIED_UNIQUE_LEN);
//    printf("--> Free Unique: %d, %d\n", PARTICLES_FREE_UNIQUE_LEN, particles_data.PARTICLES_FREE_UNIQUE_LEN);
//    assert(PARTICLES_FREE_UNIQUE_LEN == particles_data.PARTICLES_FREE_UNIQUE_LEN);
//    
//
...
//    gpuErrchk(cudaMemcpy(res_particles_free_y, d_particles_free_y, sz_particles_free_pos, cudaMemcpyDeviceToHost));
//
//    ASSERT_particles_occupied(res_particles_occupied_x, res_particles_occupied_y, 
//        particles_data.particles_occupied_unique_x.data(), particles_data.particles_occupied_unique_y.data(),
//        "Occupied", PARTICLES_OCCUPIED_UNIQUE_LEN, false);
//    ASSERT_particles_occupied(res_particles_free_x, res_particles_free_y, 
//        particles_data.particles_free_unique_x.data(), particles_data.particles_free_unique_y.data(),
...
//    gpuErrchk(cudaMalloc((void**)&d_log_odds, sz_log_odds));
//    gpuErrchk(cudaMemcpy(d_log_odds, map_data_bg.log_odds.data(), sz_log_odds, cudaMemcpyHostToDevice));
//
//
//    /************************** LOG-ODDS KERNEL *************************/
...
//        d_particles_occupied_x, d_particles_occupied_y, 2 * general_info.log_t, GRID_WIDTH, GRID_HEIGHT, PARTICLES_OCCUPIED_UNIQUE_LEN);
//    cudaDeviceSynchronize();
//
//    threadsPerBlock = 256;
//    blocksPerGrid = (PARTICLES_FREE_UNIQUE_LEN + threadsPerBlock - 1) / threadsPerBlock;
//    kernel_update_log_odds << <blocksPerGrid, threadsPerBlock >> > (d_log_odds, SEP,
//        d_particles_free_x, d_particles_free_y, (-1) * general_info.log_t, GRID_WIDTH, GRID_HEIGHT, PARTICLES_FREE_UNIQUE_LEN);
//    cudaDeviceSynchronize();
//
//    threadsPerBlock = 256;
...
//    gpuErrchk(cudaMemcpy(res_grid_map, d_grid_map, sz_grid_map, cudaMemcpyDeviceToHost));
//
//    ASSERT_log_odds(res_log_odds, map_data.log_odds.data(), map_data_post.log_odds.data(), 
//        (GRID_WIDTH * GRID_HEIGHT), (map_data.GRID_WIDTH* map_data.GRID_HEIGHT), false);
//    ASSERT_log_odds_maps(res_grid_map, map_data.grid_map.data(), map_data_post.grid_map.data(), 
//        (GRID_WIDTH * GRID_HEIGHT), (map_data.GRID_WIDTH* map_data.GRID_HEIGHT), false);
//    printf("\n");
//
...
void host_update_map_2() {
    ...
    dvec_grid_map.assign(map_data_bg.grid_map.begin(), map_data_bg.grid_map.end());
    dvec_particles_occupied_x.assign(particles_data.particles_occupied_x.begin(), particles_data.particles_occupied_x.end());
    dvec_particles_occupied_y.assign(particles_data.particles_occupied_y.begin(), particles_data.particles_occupied_y.end());
    dvec_particles_free_x.assign(particles_data.particles_free_x.begin(), particles_data.particles_free_x.end());
    dvec_particles_free_y.assign(particles_data.particles_free_y.begin(), particles_data.particles_free_y.end());

    /************************* LOG-ODDS VARIABLES ***********************/

    host_vector<int> hvec_occupied_unique_counter(1);
    host_vector<int> hvec_free_unique_counter(1);
    host_vector<int> hvec_occupied_unique_counter_col(GRID_WIDTH + 1);
    host_vector<int> hvec_free_unique_counter_col(GRID_WIDTH + 1);

    hvec_occupied_map_idx[1] = PARTICLES_OCCUPIED_LEN;
    hvec_free_map_idx[1] = PARTICLES_FREE_LEN;

    device_vector<uint8_t> dvec_occupied_map_2d(GRID_WIDTH * GRID_HEIGHT);
    device_vector<uint8_t> dvec_free_map_2d(GRID_WIDTH * GRID_HEIGHT);
    device_vector<int> dvec_occupied_unique_counter(1);
    device_vector<int> dvec_occupied_unique_counter_col(GRID_WIDTH + 1);
    device_vector<int> dvec_free_unique_counter(1);
    device_vector<int> dvec_free_unique_counter_col(GRID_WIDTH + 1);

    device_vector<int> dvec_occupied_map_idx(2, 0);
    device_vector<int> dvec_free_map_idx(2, 0);

    dvec_occupied_map_idx.assign(hvec_occupied_map_idx.begin(), hvec_occupied_map_idx.end());
    dvec_free_map_idx.assign(hvec_free_map_idx.begin(), hvec_free_map_idx.end());

    thrust::fill(dvec_occupied_unique_counter_col.begin(), dvec_occupied_unique_counter_col.end(), 0);
    thrust::fill(dvec_free_unique_counter_col.begin(), dvec_free_unique_counter_col.end(), 0);
    ...
    kernel_create_2d_map << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_occupied_map_2d), SEP,
        THRUST_RAW_CAST(dvec_particles_occupied_x), THRUST_RAW_CAST(dvec_particles_occupied_y), THRUST_RAW_CAST(dvec_occupied_map_idx),
        PARTICLES_OCCUPIED_LEN, GRID_WIDTH, GRID_HEIGHT, NUM_PARTICLES);
    kernel_create_2d_map << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_free_map_2d), SEP,
        THRUST_RAW_CAST(dvec_particles_free_x), THRUST_RAW_CAST(dvec_particles_free_y), THRUST_RAW_CAST(dvec_free_map_idx),
        PARTICLES_FREE_LEN, GRID_WIDTH, GRID_HEIGHT, NUM_PARTICLES);
    cudaDeviceSynchronize();
    auto stop_create_map = std::chrono::high_resolution_clock::now();
    ...
    kernel_2d_map_counter << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_occupied_unique_counter), THRUST_RAW_CAST(dvec_occupied_unique_counter_col), SEP,
        THRUST_RAW_CAST(dvec_occupied_map_2d), GRID_WIDTH, GRID_HEIGHT);
    kernel_2d_map_counter << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_free_unique_counter), THRUST_RAW_CAST(dvec_free_unique_counter_col), SEP,
        THRUST_RAW_CAST(dvec_free_map_2d), GRID_WIDTH, GRID_HEIGHT);
    cudaDeviceSynchronize();
    auto stop_unique_counter = std::chrono::high_resolution_clock::now();
    ...
    threadsPerBlock = 1;
    blocksPerGrid = 1;
    kernel_update_unique_sum_col << <blocksPerGrid, threadsPerBlock >> > (THRUST_RAW_CAST(dvec_occupied_unique_counter_col), GRID_WIDTH);
    kernel_update_unique_sum_col << <blocksPerGrid, threadsPerBlock >> > (THRUST_RAW_CAST(dvec_free_unique_counter_col), GRID_WIDTH);
    cudaDeviceSynchronize();
    auto stop_unique_sum = std::chrono::high_resolution_clock::now();

    hvec_occupied_map_idx.assign(dvec_occupied_map_idx.begin(), dvec_occupied_map_idx.end());

    hvec_occupied_unique_counter.assign(dvec_occupied_unique_counter.begin(), dvec_occupied_unique_counter.end());
    hvec_free_unique_counter.assign(dvec_free_unique_counter.begin(), dvec_free_unique_counter.end());
    ...
    /*---------------------------------------------------------------------*/
    /*-------------------- REINITIALIZE MAP VARIABLES ---------------------*/
    PARTICLES_OCCUPIED_UNIQUE_LEN = hvec_occupied_unique_counter[0];
    PARTICLES_FREE_UNIQUE_LEN = hvec_free_unique_counter[0];
    ...
    host_vector<int> hvec_particles_occupied_x(PARTICLES_OCCUPIED_UNIQUE_LEN);
    host_vector<int> hvec_particles_occupied_y(PARTICLES_OCCUPIED_UNIQUE_LEN);
    host_vector<int> hvec_particles_free_x(PARTICLES_FREE_UNIQUE_LEN);
    host_vector<int> hvec_particles_free_y(PARTICLES_FREE_UNIQUE_LEN);

    dvec_particles_occupied_x.clear();
    dvec_particles_occupied_y.clear();
    dvec_particles_free_x.clear();
    dvec_particles_free_y.clear();

    dvec_particles_occupied_x.resize(PARTICLES_OCCUPIED_UNIQUE_LEN, 0);
    dvec_particles_occupied_y.resize(PARTICLES_OCCUPIED_UNIQUE_LEN, 0);
    dvec_particles_free_x.resize(PARTICLES_FREE_UNIQUE_LEN, 0);
    dvec_particles_free_y.resize(PARTICLES_FREE_UNIQUE_LEN, 0);

    auto start_restructure_map = std::chrono::high_resolution_clock::now();
    threadsPerBlock = GRID_WIDTH;
    blocksPerGrid = 1;
    kernel_update_unique_restructure << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_particles_occupied_x), THRUST_RAW_CAST(dvec_particles_occupied_y), SEP,
        THRUST_RAW_CAST(dvec_occupied_map_2d), THRUST_RAW_CAST(dvec_occupied_unique_counter_col), GRID_WIDTH, GRID_HEIGHT);
    kernel_update_unique_restructure << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_particles_free_x), THRUST_RAW_CAST(dvec_particles_free_y), SEP,
        THRUST_RAW_CAST(dvec_free_map_2d), THRUST_RAW_CAST(dvec_free_unique_counter_col), GRID_WIDTH, GRID_HEIGHT);
    cudaDeviceSynchronize();
    auto stop_restructure_map = std::chrono::high_resolution_clock::now();

    hvec_particles_occupied_x.assign(dvec_particles_occupied_x.begin(), dvec_particles_occupied_x.end());
    hvec_particles_occupied_y.assign(dvec_particles_occupied_y.begin(), dvec_particles_occupied_y.end());
    hvec_particles_free_x.assign(dvec_particles_free_x.begin(), dvec_particles_free_x.end());
    hvec_particles_free_y.assign(dvec_particles_free_y.begin(), dvec_particles_free_y.end());

    ASSERT_particles_occupied(hvec_particles_occupied_x.data(), hvec_particles_occupied_y.data(),
        particles_data.particles_occupied_unique_x.data(), particles_data.particles_occupied_unique_y.data(),
        "Occupied", PARTICLES_OCCUPIED_UNIQUE_LEN, false);
    ASSERT_particles_occupied(hvec_particles_free_x.data(), hvec_particles_free_y.data(),
        particles_data.particles_free_unique_x.data(), particles_data.particles_free_unique_y.data(),
        "Free", PARTICLES_FREE_UNIQUE_LEN, false);

    /************************* LOG-ODDS VARIABLES ***********************/
    host_vector<int> hvec_grid_map(GRID_WIDTH* GRID_HEIGHT, 0);
    host_vector<float> hvec_log_odds(GRID_WIDTH* GRID_HEIGHT, 0);

    device_vector<float> dvec_log_odds(GRID_WIDTH* GRID_HEIGHT);
    dvec_log_odds.assign(map_data_bg.log_odds.begin(), map_data_bg.log_odds.end());
    ...
    kernel_update_log_odds << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_log_odds), SEP,
        THRUST_RAW_CAST(dvec_particles_occupied_x), THRUST_RAW_CAST(dvec_particles_occupied_y), 
    ...
    kernel_update_log_odds << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_log_odds), SEP,
        THRUST_RAW_CAST(dvec_particles_free_x), THRUST_RAW_CAST(dvec_particles_free_y), 
    ...
    kernel_update_map << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_grid_map), SEP,
        THRUST_RAW_CAST(dvec_log_odds), LOG_ODD_PRIOR, WALL, FREE, GRID_WIDTH * GRID_HEIGHT);
    cudaDeviceSynchronize();
    ...
    ASSERT_log_odds(hvec_log_odds.data(), map_data.log_odds.data(), map_data_post.log_odds.data(), 
        (GRID_WIDTH * GRID_HEIGHT), (map_data.GRID_WIDTH * map_data.GRID_HEIGHT), false);
    ASSERT_log_odds_maps(hvec_grid_map.data(), map_data.grid_map.data(), map_data_post.grid_map.data(), 
        (GRID_WIDTH * GRID_HEIGHT), (map_data.GRID_WIDTH* map_data.GRID_HEIGHT), false);
    printf("\n");
...
//
//    printf("xmin=%d, xmax=%d, ymin=%d, ymax=%d\n", xmin, xmax, ymin, ymax);
//    printf("xmin=%d, xmax=%d, ymin=%d, ymax=%d\n", map_data_post.xmin, map_data_post.xmax, map_data_post.ymin, map_data_post.ymax);
//    assert(EXTEND == map_data.should_extend);
//
//    if (EXTEND == true) {
//
...
//        int error_log = 0;
//        for (int i = 0; i < (GRID_WIDTH * GRID_HEIGHT); i++) {
//            if (res_grid_map[i] != map_data_bg.grid_map.at(i)) {
//                error_map += 1;
//                //printf("Grid Map: %d <> %d\n", res_grid_map[i], bg_grid_map[i]);
//            }
//            if ( abs(res_log_odds[i] - map_data_bg.log_odds.at(i)) > 1e-4) {
//                error_log += 1;
...
//    ASSERT_particles_world_frame(res_particles_world_x, res_particles_world_y, 
//        particles_data.particles_world_x.data(), particles_data.particles_world_y.data(), LIDAR_COORDS_LEN, false);
//    ASSERT_processed_measurements(res_particles_occupied_x, res_particles_occupied_y, 
//        particles_data.particles_occupied_x.data(), particles_data.particles_occupied_y.data(), LIDAR_COORDS_LEN);
//    ASSERT_position_image_body(res_position_image_body, position_transition.position_image_body.data(), true, true);
//
//    /************************* BRESENHAM KERNEL *************************/
...
//    ASSERT_particles_free_new_len(PARTICLES_FREE_LEN, particles_data.PARTICLES_FREE_LEN);
//    ASSERT_particles_free(res_particles_free_x, res_particles_free_y, 
//        particles_data.particles_free_x.data(), particles_data.particles_free_y.data(), PARTICLES_FREE_LEN);
//
//    /*---------------------------------------------------------------------*/
//    /*-------------------- REINITIALIZE MAP VARIABLES ---------------------*/
//    h_free_map_idx[1] = PARTICLES_FREE_LEN;
...
//    threadsPerBlock = 256;
//    blocksPerGrid = (PARTICLES_OCCUPIED_UNIQUE_LEN + threadsPerBlock - 1) / threadsPerBlock;
//    kernel_update_log_odds << <blocksPerGrid, threadsPerBlock >> > (d_log_odds, SEP, d_particles_occupied_x, d_particles_occupied_y, 2 * general_info.log_t, GRID_WIDTH, GRID_HEIGHT, PARTICLES_OCCUPIED_UNIQUE_LEN);
//    cudaDeviceSynchronize();
//
//    threadsPerBlock = 256;
//    blocksPerGrid = (PARTICLES_FREE_UNIQUE_LEN + threadsPerBlock - 1) / threadsPerBlock;
//    kernel_update_log_odds << <blocksPerGrid, threadsPerBlock >> > (d_log_odds, SEP, d_particles_free_x, d_particles_free_y, (-1) * general_info.log_t, GRID_WIDTH, GRID_HEIGHT, PARTICLES_FREE_UNIQUE_LEN);
//    cudaDeviceSynchronize();
//
//    threadsPerBlock = 256;
//    blocksPerGrid = ((GRID_WIDTH * GRID_HEIGHT) + threadsPerBlock - 1) / threadsPerBlock;
...
//    gpuErrchk(cudaMemcpy(res_grid_map, d_grid_map, sz_grid_map, cudaMemcpyDeviceToHost));
//
//    ASSERT_log_odds(res_log_odds, map_data.log_odds.data(), map_data_post.log_odds.data(), (GRID_WIDTH * GRID_HEIGHT), false);
//
//    ASSERT_log_odds_maps(res_grid_map, map_data.grid_map.data(), map_data_post.grid_map.data(), (GRID_WIDTH * GRID_HEIGHT), false);
//    printf("\n");
//
//
//    auto duration_world_to_image_transform_1 = std::chrono::duration_cast<std::chrono::microseconds>(stop_world_to_image_transform_1 - start_world_to_image_transform_1);
...
void host_map_2() {
    ...
    host_vector<float> hvec_transition_world_lidar(9);
    host_vector<int> hvec_particles_occupied_x(LIDAR_COORDS_LEN);
    host_vector<int> hvec_particles_occupied_y(LIDAR_COORDS_LEN);
    host_vector<float> hvec_particles_world_x(LIDAR_COORDS_LEN);
    host_vector<float> hvec_particles_world_y(LIDAR_COORDS_LEN);
    host_vector<int> hvec_position_image_body(2);
    ...
    device_vector<float> dvec_transition_single_world_body(9);
    device_vector<float> dvec_transition_single_world_lidar(9);
    device_vector<int> dvec_particles_occupied_x(LIDAR_COORDS_LEN);
    device_vector<int> dvec_particles_occupied_y(LIDAR_COORDS_LEN);
    device_vector<float> dvec_particles_world_x(LIDAR_COORDS_LEN);
    device_vector<float> dvec_particles_world_y(LIDAR_COORDS_LEN);
    ...
    host_vector<int> hvec_particles_free_counter(PARTICLE_UNIQUE_COUNTER);
    ...
    device_vector<int> dvec_occupied_map_idx(2);
    device_vector<int> dvec_free_map_idx(2);
    ...
    if (EXTEND == true) {
        ...
        kernel_position_to_image << <1, 1 >> > (THRUST_RAW_CAST(dvec_coord), SEP, xmin_pre, ymax_pre, general_info.res, xmin, ymax);
        cudaDeviceSynchronize();
        ...
        dvec_free_unique_counter_col.clear();
        dvec_free_unique_counter_col.resize(GRID_WIDTH + 1);
        host_vector<int> hvec_free_unique_counter_col(GRID_WIDTH + 1);

        dvec_occupied_unique_counter_col.clear();
        dvec_occupied_unique_counter_col.resize(GRID_WIDTH + 1);
        host_vector<int> hvec_occupied_unique_counter_col(GRID_WIDTH + 1);

        dvec_free_map_2d.clear();
        dvec_occupied_map_2d.clear();
        dvec_free_map_2d.resize(GRID_WIDTH* GRID_HEIGHT);
        dvec_occupied_map_2d.resize(GRID_WIDTH* GRID_HEIGHT);
        ...
        int error_log = 0;
        for (int i = 0; i < (GRID_WIDTH * GRID_HEIGHT); i++) {
            if (hvec_grid_map[i] != map_data_bg.grid_map.at(i)) {
                error_map += 1;
                //printf("Grid Map: %d <> %d\n", hvec_grid_map[i], bg_grid_map[i]);
            }
            if (abs(hvec_log_odds[i] - map_data_bg.log_odds.at(i)) > 1e-4) {
                error_log += 1;
                //printf("Log Odds: (%d) %f <> %f\n", i, hvec_log_odds[i], bg_log_odds[i]);
            }
            if (error_log > 200)
                break;
        }
    ...
    kernel_position_to_image << < 1, 1 >> > (
        THRUST_RAW_CAST(dvec_position_image_body), SEP, THRUST_RAW_CAST(dvec_transition_single_world_lidar), general_info.res, xmin, ymax);
    cudaDeviceSynchronize();
    auto stop_world_to_image_transform_2 = std::chrono::high_resolution_clock::now();
    ...
    ASSERT_particles_world_frame(hvec_particles_world_x.data(), hvec_particles_world_y.data(),
        particles_data.particles_world_x.data(), particles_data.particles_world_y.data(), LIDAR_COORDS_LEN, false);
    ASSERT_processed_measurements(hvec_particles_occupied_x.data(), hvec_particles_occupied_y.data(),
        particles_data.particles_occupied_x.data(), particles_data.particles_occupied_y.data(), LIDAR_COORDS_LEN);
    ASSERT_position_image_body(hvec_position_image_body.data(), position_transition.position_image_body.data(), true, true);
    ...
    kernel_bresenham << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_particles_free_x_max), THRUST_RAW_CAST(dvec_particles_free_y_max), THRUST_RAW_CAST(dvec_particles_free_counter), SEP,
        THRUST_RAW_CAST(dvec_particles_occupied_x), THRUST_RAW_CAST(dvec_particles_occupied_y), THRUST_RAW_CAST(dvec_position_image_body),
        PARTICLES_OCCUPIED_LEN, MAX_DIST_IN_MAP);
    cudaDeviceSynchronize();
    thrust::exclusive_scan(thrust::device, dvec_particles_free_counter.begin(), dvec_particles_free_counter.end(), dvec_particles_free_counter.begin(), 0); // in-place scan
    auto stop_bresenham = std::chrono::high_resolution_clock::now();

    auto start_bresenham_rearrange = std::chrono::high_resolution_clock::now();

    hvec_particles_free_counter.assign(dvec_particles_free_counter.begin(), dvec_particles_free_counter.end());
    dvec_particles_free_idx.assign(dvec_particles_free_counter.begin(), dvec_particles_free_counter.end());
    ...
    hvec_particles_occupied_x.resize(PARTICLES_OCCUPIED_UNIQUE_LEN, 0);
    hvec_particles_occupied_y.resize(PARTICLES_OCCUPIED_UNIQUE_LEN, 0);
    hvec_particles_free_x.resize(PARTICLES_FREE_UNIQUE_LEN, 0);
    hvec_particles_free_y.resize(PARTICLES_FREE_UNIQUE_LEN, 0);
    
    dvec_particles_occupied_x.resize(PARTICLES_OCCUPIED_UNIQUE_LEN);
    dvec_particles_occupied_y.resize(PARTICLES_OCCUPIED_UNIQUE_LEN);
    dvec_particles_free_x.resize(PARTICLES_FREE_UNIQUE_LEN);
    dvec_particles_free_y.resize(PARTICLES_FREE_UNIQUE_LEN);
    ...
    kernel_update_log_odds << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_log_odds), SEP, 
        THRUST_RAW_CAST(dvec_particles_occupied_x), THRUST_RAW_CAST(dvec_particles_occupied_y), 2 * general_info.log_t, GRID_WIDTH, GRID_HEIGHT, PARTICLES_OCCUPIED_UNIQUE_LEN);
    cudaDeviceSynchronize();
    ...
    kernel_update_log_odds << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_log_odds), SEP, 
        THRUST_RAW_CAST(dvec_particles_free_x), THRUST_RAW_CAST(dvec_particles_free_y), (-1) * general_info.log_t, GRID_WIDTH, GRID_HEIGHT, PARTICLES_FREE_UNIQUE_LEN);
    cudaDeviceSynchronize();

    threadsPerBlock = 256;
    blocksPerGrid = ((GRID_WIDTH * GRID_HEIGHT) + threadsPerBlock - 1) / threadsPerBlock;
    kernel_update_map << <blocksPerGrid, threadsPerBlock >> > (
        THRUST_RAW_CAST(dvec_grid_map), SEP, THRUST_RAW_CAST(dvec_log_odds), LOG_ODD_PRIOR, WALL, FREE, GRID_WIDTH * GRID_HEIGHT);
    cudaDeviceSynchronize();
    auto stop_update_map = std::chrono::high_resolution_clock::now();

    thrust::fill(hvec_log_odds.begin(), hvec_log_odds.end(), 0);
    hvec_log_odds.assign(dvec_log_odds.begin(), dvec_log_odds.end());
    hvec_grid_map.assign(dvec_grid_map.begin(), dvec_grid_map.end());
