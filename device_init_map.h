...
void alloc_init_particles_vars(DeviceParticles& d_particles, HostParticles& h_particles,
    ...
    d_particles.v_free_counter.resize(h_particles.MAX_OCCUPIED_LEN, 0);
    d_particles.v_free_idx.resize(h_particles.MAX_OCCUPIED_LEN, 0);

    d_particles.s_free_x_max.resize(h_particles.MAX_OCCUPIED_LEN * MAX_DIST_IN_MAP, 0);
    d_particles.s_free_y_max.resize(h_particles.MAX_OCCUPIED_LEN * MAX_DIST_IN_MAP, 0);
    ...
    h_particles.v_free_counter.resize(h_particles.MAX_OCCUPIED_LEN, 0);
...
void alloc_init_unique_map_vars(Device2DUniqueFinder& d_unique,
    ...
    d_unique.c_in_map.resize(1);
    d_unique.s_in_col.resize(h_map.GRID_WIDTH + 1);
    d_unique.c_idx.resize(2);

    d_unique.c_idx.assign(hvec_map_idx.begin(), hvec_map_idx.end());

    res_unique.c_in_map.resize(1);
    res_unique.s_in_col.resize(h_map.GRID_WIDTH + 1);
    res_unique.c_idx.resize(2);
}
